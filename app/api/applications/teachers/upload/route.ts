...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    const { data: files, error } = await supabase
      ...
      .select(
        'id, normalized_file_name, original_file_name, file_size_bytes, drive_web_view_link, created_at'
      )
  ...
  try {
    ...
    for (const file of files) {
      ...
      const { data: insertedFile, error: dbError } = await supabase
        ...
        .select(
          'id, normalized_file_name, original_file_name, file_size_bytes, drive_web_view_link'
        )
        .single();
      ...
      if (dbError || !insertedFile) {
        ...
        return jsonError(
          500,
          'FILE_DB_SAVE_FAILED',
          'Файл загружен, но не удалось сохранить его в системе.'
        );
      }
