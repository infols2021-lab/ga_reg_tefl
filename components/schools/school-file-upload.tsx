...
type SchoolFileUploadProps = {
  applicationId: string | null;
  programType: 'primary' | 'secondary';
  onUploaded?: () => void;
...
export default function SchoolFileUpload({
  ...
  const uploadApi = `/api/applications/${programType}/upload`;
  const deleteApi = `/api/applications/${programType}/delete-file`;
  ...
  async function loadUploadedFiles(currentApplicationId: string) {
    try {
      ...
      const res = await fetch(
        `${uploadApi}?applicationId=${encodeURIComponent(currentApplicationId)}`,
