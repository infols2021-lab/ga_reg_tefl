...
export default function Step3Review({
  ...
  const policyLink = process.env.NEXT_PUBLIC_PRIMARY_POLICY_PDF_URL || '#';
  const termsLink = process.env.NEXT_PUBLIC_PRIMARY_TERMS_PDF_URL || '#';
        ...
        <div className="mt-4 space-y-4">
          ...
          <label className="flex items-start gap-3">
            <input
              ...
              onChange={(e) => onChange('consentPersonalData', e.currentTarget.checked)}
              className="mt-1 h-4 w-4 shrink-0 accent-black"
            />
          ...
          <label className="flex items-start gap-3">
            <input
              ...
              onChange={(e) => onChange('consentPdDistribution', e.currentTarget.checked)}
              className="mt-1 h-4 w-4 shrink-0 accent-black"
            />
            <span className="text-sm leading-6 text-slate-700">
              Я ознакомлен и согласен с{' '}
